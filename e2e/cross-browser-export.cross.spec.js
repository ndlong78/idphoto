...
async function uploadGeneratedPortrait(page, filename = 'cross-browser-portrait.png') {
...
test('upload → readiness dialog → JPG 300 DPI → receipt', async ({ page, browserName }) => {
  ...
  expect(bytes.length, `${browserName} phải tạo JPEG có dữ liệu`).toBeGreaterThan(1_000);
...
test('upload → checklist/audit → ZIP chứa JPEG và audit riêng tư', async ({ page }) => {
  await uploadGeneratedPortrait(page, 'private-cross-browser-source.png');
  await waitForEditor(page);
  ...
  expect(JSON.stringify(audit)).not.toContain('private-cross-browser-source.png');
